...
final class Administration extends UserManager {
	 ...
	 * @throws AuthError if an internal problem occurred (do *not* catch)
	 */
        public function EditPage($heading,$content){
 
		$pageEdit = $this->EditContent($heading,$content);
		return $pageEdit;
	...
	public function GetPageContent(){
		...
		$page_con = $this->db->select('select page_name,heading,content from' . $this->dbTablePrefix . '`page'); 
...
/**
*Add new page with user heading and content
*
*
*/
	public function AddNewPage($username,$heading , $content , $active , $page_name ){	
$InsertPage = $this->InsertNewPage($username, $heading , $content , $active, $page_name);
if ($insertPage === 0) {
...
/**
*Insert New page user heading content 
*
*
*
*/
private function EditContent($heading,$content){
try
{
echo "stage 2 editing content";
$updateingpage = $this->db->exec('UPDATE ' . $this->dbTablePrefix . ' page SET content = ? where page_name = ?' , [$content , $heading ] );
print $updateingpage;
}catch (Error $e) {
                        throw new DatabaseError($e->getMessage());
                }
echo "Should be edited";

}
private function InsertNewPage($username , $heading , $content , $active, $page_name) {
try {
	$page = $this->db->selectValue('SELECT page_name FROM ' . $this->dbTablePrefix . 'page where page_name = ?' , [$heading]);
}
                catch (Error $e) {
                        throw new DatabaseError($e->getMessage());
                }
...
$page = $this->db->insert($this->dbTablePrefix . 'page' , [ 'user' => $username ,'heading' => $heading , 'content' => $content  , 'active' => $active , 'page_name' => $page_name ] );
}
catch (Error $e) {
                        throw new DatabaseError($e->getMessage());
                }
	...
	private function logInAsUserByColumnValue($columnName, $columnValue) {
		try {
			$users = $this->db->select(
				'SELECT heading FROM ' . $this->dbTablePrefix . 'users WHERE ' . $columnName . ' = ? LIMIT 2 OFFSET 0',
				[ $columnValue ]
			);
